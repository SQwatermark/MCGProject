...
public class RenderGRMotor extends AbsCartRenderer {

    public static IRenderFactory<GRMotor> FACTORY = RenderGRMotor::new;

    public RenderGRMotor(RenderManager manager) {
        super(manager);
    }

    private final String _name = "grbm";
    ...
    public void render(EntityMinecart cart, double x, double y, double z, float yaw) {
        ...
        float angle = 0.0F;
        if (cart instanceof GRMotor) {
            angle = ((GRMotor)cart).angle;
        }
